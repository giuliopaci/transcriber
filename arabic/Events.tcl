...
proc CreateWho {bp} {
   ...
   foreach nb {1 2} {
      if {$nb == "1"} {
	 $v(tk,edit) mark set insert "$bp.last lineend"
	 set data [GetDataFromPos "insert"]
	 $t tag remove $data "insert"
	 set tag [::xml::element "Who" [list "nb" $nb] -after $bp]
      } else {
	 set last [lindex [$t tag nextrange "sync" "$bp.first"] 0]
...
proc InsertWho {elem {other_tags ""}} {
   ...
   if {$nb > 1} {
      ...
      $t mark set insert "insert-6c"
   } else {
      $t insert "insert" " :$nb" [concat "sync" "locked" $elem $other_tags]
      $t mark set insert "insert-3c"
   }
...
proc CreateBackground {} {
   ...
   if {($back0 != "" && [$t compare "$back0.last linestart" == "insert linestart"] && [$t compare $back0.last < insert])
    || ($back1 != "" && [$t compare "$back1.last linestart" == "insert linestart"] && [$t compare $back1.first > insert])} {
...
proc EditBackground {back} {
   ...
   tkTextSetCursor $v(tk,edit) "$back.first"
...
proc CreateEvent {txt {type "noise"} {extent "instantaneous"} {interactif 0}} {
   ...
   set v(sel1) $sel
   set v(sel2)  [lindex [$t tag ranges sel] 1]
   if {$sel != ""} {
      set extent "begin"
...
proc EditEvent {tag {mode "Edit"} {sel ""}} {
   ...
   set rads [RadioFrame $w.typ "Type" v(type,chosen) {
      "Noise" "Comment" "\t" "Pronounce" "Lexical" "Language"
   } {
      "noise" "comment" "\t" "pronounce" "lexical" "language"
   }]
   ...
   switch [OkCancelModal $w $e $buttons($mode)] {
	 ...
	 set tag [CreateEvent $v(desc,chosen) $v(type,chosen) $v(extn,chosen)]
      ...
      "Destroy" {
         set extent [$tag getAttr "extent"]
	 if {$extent == "begin"} {
	     set end [$t search "\[" "$tag.last"] 
	     set ent_color_tag [lindex [$t tag names $tag.last+1c] end-1]
	     if {[lsearch $list_ent_tag $ent_color_tag] != -1} {
		 $t tag remove $ent_color_tag $tag.last $end         
	     }
	     set tag_begin_desc [$tag getAttr "desc"]
	     set tag_end [lindex [$t tag names "$end"] end]
	     set tag_end_extent [$tag_end getAttr "extent"]
	     set tag_end_desc [$tag_end getAttr "desc"]
	     if {$tag_end_extent=="end" && $tag_end_desc=="$tag_begin_desc"} {
		 SuppressEvent $tag_end
	     }
	 }
	 if {$extent == "end"} {
	     set start [$t search -backward "\]" $tag.first]
	     set ent_color_tag [lindex [$t tag names $tag.first-1c] end-1]
	     if {[lsearch $list_ent_tag $ent_color_tag] != -1} {
		 $t tag remove $ent_color_tag "$start+1c" $tag.first         
	     }
	     set tag_end_desc [$tag getAttr "desc"]
	     set tag_begin [lindex [$t tag names "$start-1c"] end]
	     set tag_begin_extent [$tag_begin getAttr "extent"]
	     set tag_begin_desc [$tag_begin getAttr "desc"]
	     if {$tag_begin_extent=="begin" && $tag_begin_desc=="$tag_end_desc"} {
		 SuppressEvent $tag_begin
	     }
	 }
	 SuppressEvent $tag
	 set tag ""
...
proc ConfigureEvents {} {
   ...
   pack $h -fill both -expand true -side left
   foreach title {
    "Comment" "Noise" "Pronounce" "Lexical" "Language"
   } var {
      "comment" "noise" "pronounce" "lexical" "language"
   } {
      set e [EntryFrame $h.$var $title v(event,$var)]
